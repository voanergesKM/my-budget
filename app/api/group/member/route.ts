...
import { deleteUserFeromGroup } from "@/app/lib/db/controllers/userController";
...
export const DELETE = wrapPrivateHandler(async (req: NextRequest, token) => {
  ...
  if (memberId && groupId && origin) {
    if (origin === "pending") {
      const data = await deleteMemberInvitation(memberId, token);

      return NextResponse.json({ success: true, data }, { status: 200 });
    } else if (origin === "member") {
      const data = await Promise.all([
        deleteGroupMember(groupId, memberId, token),
        deleteUserFeromGroup(groupId, memberId, token),
      ]);
